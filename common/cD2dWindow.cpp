...
// static
//{{{
bool cD2dWindow::resolveShortcut (const string& shortcut, string& fullName) {
...
//}}}
//{{{
void cD2dWindow::scanDirectory (vector<string>& fileNames, const string& parentName,
  ...
  if (file != INVALID_HANDLE_VALUE) {
    do {
      if ((findFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) && (findFileData.cFileName[0] != '.'))
        scanDirectory (fileNames, pathFileName, findFileData.cFileName, pathMatchName);
      else if (PathMatchSpecA (findFileData.cFileName, pathMatchName.c_str()))
        if ((findFileData.cFileName[0] != '.') && (findFileData.cFileName[0] != '..'))
          fileNames.push_back (pathFileName + "/" + findFileData.cFileName);
...
//}}}
//{{{
vector<string> cD2dWindow::getFiles (const string& fileName, const string& match) {
  ...
  if (GetFileAttributesA (useFileName.c_str()) & FILE_ATTRIBUTE_DIRECTORY)
    scanDirectory (fileNames, "", useFileName, match);
  else if (!useFileName.empty())
    fileNames.push_back (useFileName);
...
//}}}
//{{{
void cD2dWindow::addBox (cBox* box, cPoint pos) {
  mBoxes.push_back (box);
  box->setPos (pos);
  }
//}}}
//{{{
void cD2dWindow::addBox (cBox* box, float x, float y) {
  addBox (box, cPoint(x,y));
  }
//}}}
//{{{
void cD2dWindow::addBox (cBox* box) {
  addBox (box, cPoint());
  }
//}}}
//{{{
void cD2dWindow::addBoxBelow (cBox* box) {
...
void cD2dWindow::createDirect2d() {
  ...
  mDeviceContext->CreateSolidColorBrush (ColorF (0.f, 0.f, 0.f, 0.8f), &mTransparentBlackBrush);
  }
//}}}
//{{{
