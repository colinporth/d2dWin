...
static void cam_to_cielab (uint16_t cam[3], float lab[3]) /* [SA] */
{
    ...
    if (cam == NULL) {
        for (i=0; i < 0x10000; i++) {
            r = i / 65535.0f;
            cbrt[i] = r > 0.008856f ? powf(r,1.0f/3.0f) : 7.787f*r + 16.0f/116.0f;
        }
        for (i=0; i < 3; i++)
            for (j=0; j < 3; j++)                           /* [SA] */
...
//}}}
//{{{
static void ClearBorders (uint8_t *rgb, int sx, int sy, int w)
{
    int i, j;
...
//}}}
//{{{
static void ClearBorders16 (uint16_t* rgb, int sx, int sy, int w)
{
    int i, j;
...
// such as some Basler and Point Grey camera. Most of the algos presented here come
// from http://www-ise.stanford.edu/~tingchen/ and have been converted from Matlab to C
// and extended to all elementary patterns.
// 8-bits versions, insprired by OpenCV's Bayer decoding
eBayerResult NearestNeighbor8 (const uint8_t* bayer, uint8_t* rgb, int sx, int sy, int tile)
{
    ...
    int width = sx;
    int height = sy;
    int blue = tile == COLOR_BGGR
        || tile == COLOR_GBRG ? -1 : 1;
    int start_with_green = tile == COLOR_GBRG
        || tile == COLOR_GRBG;
    int i, imax, iinc;

    if ((tile>COLOR_MAX)||(tile<COLOR_MIN))
      return INVALID_COLOR_FILTER;
...
eBayerResult Simple8 (const uint8_t* bayer, uint8_t* rgb, int sx, int sy, int tile)
{
    const int bayerStep = sx;
    const int rgbStep = 3 * sx;
    int width = sx;
...
eBayerResult Bilinear8 (const uint8_t* bayer, uint8_t* rgb, int sx, int sy, int tile)
{
    const int bayerStep = sx;
    const int rgbStep = 3 * sx;
    int width = sx;
    ...
    /*
       ...
       CV_BayerBG2BGR <-> COLOR_BGGR
       CV_BayerGB2BGR <-> COLOR_GBRG
       CV_BayerGR2BGR <-> COLOR_GRBG
...
// Henrique S. Malvar, Li-wei He, and Ross Cutler, in ICASSP'04
eBayerResult HQLinear8 (const uint8_t* bayer, uint8_t* rgb, int sx, int sy, int tile)
{
    const int bayerStep = sx;
    const int rgbStep = 3 * sx;
    int width = sx;
    int height = sy;
    int blue = tile == COLOR_BGGR || tile == COLOR_GBRG ? -1 : 1;
    int start_with_green = tile == COLOR_GBRG || tile == COLOR_GRBG;
...
eBayerResult VNG8 (const uint8_t* bayer, uint8_t * dst, int sx, int sy, tBayerOrder pattern)
{
    ...
    Bilinear8 (bayer, dst, sx, sy, pattern);
...
eBayerResult AHD8 (const uint8_t* bayer, uint8_t * dst, int sx, int sy, tBayerOrder pattern)
{
...
// 16bit
//{{{
eBayerResult NearestNeighbor16 (const uint16_t* bayer, uint16_t* rgb, int sx, int sy, int tile, int bits)
{
    const int bayerStep = sx;
    const int rgbStep = 3 * sx;
    int width = sx;
...
//}}}
//{{{
eBayerResult Simple16( const uint16_t* bayer, uint16_t* rgb, int sx, int sy, int tile, int bits)
{
...
//}}}
//{{{
eBayerResult Bilinear16 (const uint16_t* bayer, uint16_t* rgb, int sx, int sy, int tile, int bits)
{
    const int bayerStep = sx;
    const int rgbStep = 3 * sx;
    int width = sx;
...
//}}}
//{{{
eBayerResult HQLinear16 (const uint16_t* bayer, uint16_t* rgb, int sx, int sy, int tile, int bits)
{
    const int bayerStep = sx;
    const int rgbStep = 3 * sx;
    int width = sx;
...
//}}}
//{{{
eBayerResult VNG16 (const uint16_t* bayer, uint16_t * dst, int sx, int sy, tBayerOrder pattern, int bits)
{
    const int height = sy, width = sx;
    static const signed char *cp;
    /* the following has the same type as the image */
    ...
    Bilinear16(bayer, dst, sx, sy, pattern, bits);
...
//}}}
//{{{
eBayerResult AHD16 (const uint16_t* bayer,
                        uint16_t * dst, int sx, int sy,
                        tBayerOrder pattern, int bits)
{
    int i, j, top, left, row, col, tr, tc, fc, c, d, val, hm[2];
    /* the following has the same type as the image */
...
//{{{
eBayerResult bayerDecode8 (const uint8_t* bayer, uint8_t* rgb, uint32_t sx, uint32_t sy, tBayer tile, tBayer method)
{
    switch (method) {
      case NEAREST:
        return NearestNeighbor8 (bayer, rgb, sx, sy, tile);
      case SIMPLE:
        return Simple8 (bayer, rgb, sx, sy, tile);
      case BILINEAR:
        return Bilinear8 (bayer, rgb, sx, sy, tile);
      case HQLINEAR:
        return HQLinear8 (bayer, rgb, sx, sy, tile);
      case VNG:
        return VNG8 (bayer, rgb, sx, sy, tile);
      case AHD:
        return AHD8 (bayer, rgb, sx, sy, tile);
      default:
        return INVALID_bayerMETHOD;
  }

}
//}}}
//{{{
eBayerResult bayerDecode16 (const uint16_t* bayer, uint16_t* rgb, uint32_t sx, uint32_t sy, tBayer tile, tBayer method, uint32_t bits)
{
    switch (method) {
      case NEAREST:
        return NearestNeighbor16 (bayer, rgb, sx, sy, tile, bits);
      case SIMPLE:
        return Simple16 (bayer, rgb, sx, sy, tile, bits);
      case BILINEAR:
        return Bilinear16 (bayer, rgb, sx, sy, tile, bits);
      case HQLINEAR:
        return HQLinear16 (bayer, rgb, sx, sy, tile, bits);
      case VNG:
        return VNG16 (bayer, rgb, sx, sy, tile, bits);
      case AHD:
        return AHD16 (bayer, rgb, sx, sy, tile, bits);
      default:
        return INVALID_bayerMETHOD;
    }
