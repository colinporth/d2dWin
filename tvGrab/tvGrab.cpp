...
int main(int argc, char** argv) {
  ...
  auto frequency = (argc == 1 ? 674 : atoi(argv[1])) * 1000;

  string fileName = "c:/videos/tune.ts";
  auto file = CreateFile (fileName.c_str(), GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, 0, NULL);
  cLog::log (LOGNOTICE, "created file " + fileName);

  auto ts = new cTransportStream();
  cLog::log (LOGNOTICE, "created ts");

  auto bda = new cBda();
  bda->createGraph (frequency, fileName);
  cLog::log (LOGNOTICE, "created bda - tuned " + dec(frequency) + "mhz");

  thread ([=]() { bdaCaptureThread (ts, bda, file); }).detach();
  thread ([=]() { bdaStrengthDiscontinuityThread (ts, bda); }).detach();
