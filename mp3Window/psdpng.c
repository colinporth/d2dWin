...
static void transform_signal() {
  ...
  short* inbuf = (short*)malloc (sizeof(short) * 2 * nfft);
  ...
  float* mag2buf = (float*)calloc (nfreqs, sizeof(float));
...
static void make_png() {
  ...
  png_set_IHDR (png_ptr, info_ptr, nfreqs, nrows,
  ...
  row_pointers = malloc(nrows*sizeof(png_bytep));
  for (i=0;i<nrows;++i) {
