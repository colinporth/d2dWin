...
mfxStatus CDecodingPipeline::CreateHWDevice()
{
  ...
  sts = m_hwdev->Init (window, 1, MSDKAdapter::GetNumber(m_mfxSession));
...
mfxStatus CDecodingPipeline::AllocFrames() {
    ...
    // TODO: workaround for P210, delete later
    if (m_mfxVideoParams.mfx.FrameInfo.FourCC != MFX_FOURCC_P210)
    {
    ...
    if (m_nMaxFps) // Add surfaces for rendering smoothness
...
template <typename Buffer> mfxStatus CDecodingPipeline::AllocateExtBuffer()
{
