...
// timers
//{{{
template <int tag = 0> class CTimeInterval : private no_copy
{
...
template <typename Buf_t, typename Length_t> bool skip(const Buf_t *&buf, Length_t &length, Length_t step)
{
    if (length < step)
        return false;
...
template<typename T> mfxStatus msdk_opt_read(const msdk_char* string, T& value);
//{{{
template<size_t S> mfxStatus msdk_opt_read(const msdk_char* string, msdk_char (&value)[S])
    {
        value[0]=0;
...
//}}}
//{{{
template<typename T> inline mfxStatus msdk_opt_read(const msdk_string& string, T& value)
    {
