...
void Node::parseAttributes() {
  ...
  while (*ptr && *ptr != '>') {
    ...
    // skip unrecognized
    while (*ptr && !isalnum (*ptr) && !isspace (*ptr))
      ++ptr;
    ...
    if (*ptr == '=') {
      ...
      if (*ptr == '"' || *ptr == '\'') {
        ...
        while (isspace (*trimmed_end) && trimmed_end >= begin)
          --trimmed_end;
      ...
      printf ("attribute insert %s = %s\n", key.c_str(), val.c_str());
      mAttributes.insert (make_pair (key, val));
      }

    else {
      printf ("attribute insert D: %s\n", key.c_str());
      if (!key.empty())
...
Node::operator string() const {
  ...
  if (isTag())
    return this->tagName() + " atts:" + to_string(mAttributes.size()) + " - text:" + this->text();
